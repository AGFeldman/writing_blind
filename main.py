...
with open(cur_path + 'backup_path.txt', 'rU') as file_:
    backup_path = file_.readline().strip()
...
def crack(file_path, file_name, key=pphrase):
    ...
    file_list = listdir(remove_slashes(file_path)+'.')
    decryptSuccess = remove_slashes(file_name) in file_list
...
def backup(file_path, file_name):
    ...
    full_name = file_path + file_name
    full_name_nc = full_name + '.nc'
    backup = backup_path + file_name + '.nc'
    copy2(remove_slashes(full_name_nc), remove_slashes(backup))

def open_(file_path, file_name, open_with, should_backup, key=pphrase):
    ...
    if path.exists(remove_slashes(full_name + '.nc')):
        ...
        open_helper(file_path, file_name, open_with)
...
if action == 'normal' or action == 'o':
    if action == 'normal':
        src_dict = file_list[0][1]
    elif action == 'o':
        for i in range(len(file_list)):
            output = str(i) + '     ' + file_list[i][0]
            print output
        num = int(raw_input('Choose a file number: '))
        src_dict = file_list[num][1]
    file_path = src_dict['path']
    file_name = src_dict['name']
    opener = src_dict['openwith']
    should_backup = int(src_dict['backup'])
